...
local function init_palette()
    ...
    if not palette then
        ...
        palette = vim.tbl_extend("force", { none = "NONE" }, palette, require("config.scheme").palette_overwrite)
    end
...
function M.gen_neodim_blend_attr()
    local trans_bg = require("config.scheme").transparent_background
    local appearance = require("config.scheme").background
